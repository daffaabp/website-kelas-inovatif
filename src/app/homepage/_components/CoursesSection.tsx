...
import { BookOpen, Laptop, Users, Handshake, Trophy, RefreshCcw, ArrowRight } from 'lucide-react';
                ...
                <div className="w-full py-16 px-4 pt-2 md:px-8 rounded-3xl shadow-xl border bg-white/50 backdrop-blur-sm" style={{ minHeight: '650px' }}>
                    ...
                    <div className="hidden xl:block relative w-full h-[550px] mx-auto -translate-x-6" style={{ maxWidth: '1050px' }}>
                        ...
                        */}
                        ...
                        <div className="absolute top-[100px] left-[100px] -translate-x-1/2 -translate-y-1/2 z-10" onClick={() => onCourseSelect?.(courses[0])}>
                            <NodeCard icon={BookOpen} title="Kelas Literature Review" sub="Riset Otomatis" color="blue" />
                        </div>
                        <div className="absolute top-[250px] left-[100px] -translate-x-1/2 -translate-y-1/2 z-10" onClick={() => onCourseSelect?.(courses[0])}>
                            <NodeCard icon={BookOpen} title="Kelas Writing" sub="Anti-Plagiasi" color="indigo" />
                        </div>
                        <div className="absolute top-[400px] left-[100px] -translate-x-1/2 -translate-y-1/2 z-10" onClick={() => onCourseSelect?.(courses[0])}>
...
function NodeCard({ icon: Icon, title, sub, color }: NodeCardProps) {
    const colorClasses = {
        blue: 'bg-blue-50 text-blue-600 border-blue-100', // Added comma for consistency/safety if needed, though last item doesnt strictly need it
    ...
    return (
        <div className="bg-white px-5 py-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4 w-full md:w-52 hover:border-gray-300 hover:shadow-md transition-all cursor-pointer group">
